...
impl<'smt, 'ctx> NeutralsRemover<'smt, 'ctx> {
    ...
    fn with_sat<T>(&mut self, expr: &Expr, callback: impl FnOnce(&mut Self) -> T) -> Option<T> {
        ...
        // Add any new assumptions produced by translating `expr`.
        self.translate
            .local_scope()
    ...
    /// Evaluate `callback` assuming `expr` is not bottom. Returns `None` if
    /// `expr` is statically bottom, skipping the callback.
    ...
    /// Evaluate `callback` assuming `expr` is not top. Returns `None` if
    /// `expr` is statically top, skipping the callback.
                ...
                match bin_op.node {
                    ...
                    BinOpKind::And => {
                        ...
                        // 3) if lhs => rhs, remove rhs
                        {
                            ...
                            let lhs_and_not_rhs =
                                builder.binary(BinOpKind::And, Some(ty), lhs.clone(), not_rhs);

                            // if lhs && !rhs is unsat, lhs => rhs holds
                    ...
                    // OR: remove false
                    Or => {
                        // top(lhs) -> result is top (infinity is the absorber of Sup)
                        let nontop: Option<Result<(), LimitError>> =
                            self.with_nontop(lhs, |_| Ok(()));
                        if nontop.is_none() {
                            let b = ExprBuilder::new(e.span);
